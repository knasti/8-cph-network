    ...
    def update_moving_costs(self):
        ...
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute("SELECT time_const, pk FROM merged_ways \
                            WHERE connector = 0 AND transport = 'metro'")
            metro_time_const_costs = cursor.fetchall()  # Stores the result of the query in the metro_data variable
            time_const = [] # Makes sure the list is empty
            time_const_id = [] # Makes sure the list is empty
            if metro_time_const_costs:
                for i in range(len(metro_time_const_costs)): # Iterating through all of the metro data
                    time_const.append(metro_time_const_costs[i][0]) # Removing the tuples that comes along with the DB queries
                    time_const_id.append(metro_time_const_costs[i][1]) # Removing the tuples that comes along with the DB queries
